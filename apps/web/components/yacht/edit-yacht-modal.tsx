...
export function EditYachtModal({ yacht, onClose, onSuccess }: EditYachtModalProps) {
  const [name, setName] = useState(yacht.name);
  const [flag, setFlag] = useState(normalizeFlagCode(yacht.flag));
  const [imoOptional, setImoOptional] = useState(yacht.imoOptional || '');
  const [isActive, setIsActive] = useState(yacht.isActive ?? true);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    setName(yacht.name);
    setFlag(normalizeFlagCode(yacht.flag));
    setImoOptional(yacht.imoOptional || '');
    setIsActive(yacht.isActive ?? true);
  }, [yacht]);

  const flagOptions = useMemo(() => {
    const current = normalizeFlagCode(flag);
    const exists = FLAG_OPTIONS.some((option) => option.code === current);

    if (!current || exists) {
      return FLAG_OPTIONS;
    }

    return [{ code: current, country: current, emoji: '' }, ...FLAG_OPTIONS];
  }, [flag]);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const payload: UpdateYachtRequest = {
        name: name.trim(),
      ...
      await api.patch(`/yachts/${yacht.id}`, payload);
      await onSuccess();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'No se pudo actualizar el yate');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="h-screen w-screen overflow-y-auto border-border bg-surface shadow-2xl sm:h-auto sm:w-full sm:max-w-lg sm:rounded-xl sm:border">
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <h2 className="text-lg font-semibold text-text-primary">Editar yate</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-4">
          ...
          <div className="rounded-lg border border-border bg-background px-3 py-2 text-xs text-text-secondary">
            Vista previa: <span className="font-medium text-text-primary">{formatFlagLabel(flag)}</span>
          </div>
